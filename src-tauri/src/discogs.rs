...
use crate::authentication_manager::AuthManager;
use crate::youtube;
...
async fn get_data_from_response(url: &str) -> Value {
    ...
    let client = reqwest::Client::builder().user_agent("chic app").build();
    let res = client
...
async fn get_release_information(want_list: &Value) -> DiscogsRelease {
    ...
    let artist = want_list["basic_information"]["artists"][0]["name"]
        .as_str()
        .to_string();
    let label = want_list["basic_information"]["label"][0]["name"]
        .as_str()
        .to_string();
    let genres = want_list["basic_information"]["genres"]
        .as_str()
        .to_string();
...
pub async fn get_want_list_information(username: String) -> String {
    ...
    let album_cover = get_album_cover(username.clone());
...
pub fn get_album_cover(title: String) -> Url {
    println!("get album covers");
    let auth_manager = AuthManager::init("discogs");
    let auth_url = auth_manager.client.auth_url().to_string();
    println!("this is the auth url {}", auth_url);
    Url::parse("http://yeah").unwrap()
}
