  ...
  pub fn init(api_name: &str) -> Self {
    ...
    match api_name {
      "google" => {
        ...
        auth_url =
          AuthUrl::new(env::var("GOOGLE_AUTH_URL").expect("Missing the GOOGLE_AUTH_URL env var"))
            .unwrap();
        token_url = Some(
          TokenUrl::new(
      ...
      "discogs" => {
        ...
        .unwrap();
        client_secret = Some(ClientSecret::new(
          env::var("DISCOGS_CLIENT_SECRET").expect("Missing the DISCOGS_CLIENT_SECRET env var"),
        ));
        auth_url =
          AuthUrl::new(env::var("DISCOGS_AUTH_URL").expect("Missing the DISCOGS_AUTH_URL env var"))
            .unwrap();
        token_url = Some(
          TokenUrl::new(
            env::var("GOOGLE_TOKEN_URL").expect("Missing the GOOGLE_TOKEN_URL env var"),
    ...
    let client = BasicClient::new(client_id, client_secret, auth_url, token_url)
      .set_redirect_uri(
        RedirectUrl::new(env::var("REDIRECT_URL").expect("Missing the REDIRECT_URL env var"))
          .expect("Invalid redirect URL"),
      )
      .set_revocation_uri(
        RevocationUrl::new(env::var("REVOCATION_URL").expect("Missing the REVOCATION_URL env var"))
